...
namespace Compori.Shipping.Dhl.Common
{
    public class Client<TSettings> where TSettings : Settings
    {
        ...
        /// Der Logger
        /// </summary>
        private static readonly NLog.Logger Log = NLog.LogManager.GetLogger(typeof(Client<TSettings>).FullName);

        /// <summary>
        /// Liefert Faktoryobjekt für das Ermitteln der Einstellungen zurück.
        /// </summary>
        /// <value>Das Faktoryobjekt für die Einstellungen.</value>
        ...
        public TSettings Settings => settings ??= SettingsFactory.Create();

        /// <summary>
        /// Liefert Faktoryobjekt für den Restclient zurück.
        /// </summary>
        /// <value>Das Faktoryobjekt für den Restclient.</value>
        ...
        /// </summary>
        /// <param name="settingsFactory">Das Faktoryobjekt für die Einstellungen.</param>
        /// <param name="restClientFactory">Das Faktoryobjekt für den Restclient.</param>
        ...
        */
        ...
        private void ProcessResponse(Settings settings, RestResponse response, DateTime startTime)
        {
            ...
            {
                throw new DhlException("You've reached a maximum that is defined in your current plan. Please upgrade to a higher plan.");
            }
            // 403
            if (response.StatusCode == HttpStatusCode.Forbidden)
            {
                throw new DhlException("You are not allowed to talk to this endpoint. This can either be due to a wrong authentication or when you're trying to reach an endpoint that your account isn't allowed to access.");
            }
            // 500
            //if (response.StatusCode == HttpStatusCode.InternalServerError)
            //{
            //    throw new DhlException($"Something has seriously gone wrong. Don't worry, we'll have a look at it. If the error persists, please don't hesitate to contact us by sending us an email containing the X-Request-ID ({response.GetRequestId()}) header we've returned.");
            //}
            ...
            {
                throw new DhlException(string.Join(" ", errorResponse.Errors));
            }
            */
        ...
        /// <param name="urlParameters">The URL pararamters.</param>
            ...
            CancellationToken cancellationToken = default) where TInput : class
        {
            ...
            return await Execute<TOutput>(request.AddJsonBody(data), Method.Post, cancellationToken).ConfigureAwait(false);
        }
        ...
        public async Task<Response<TOutput>> Post<TOutput>(string uri, CancellationToken cancellationToken = default) where TOutput : class
        {
            Guard.AssertArgumentIsNotNullOrWhiteSpace(uri, nameof(uri));

            return await Execute<TOutput>(new RestRequest(uri), Method.Post, cancellationToken).ConfigureAwait(false);
        }
        ...
        public async Task<Response> Delete(string uri, Dictionary<string, string> urlParameters = null, Dictionary<string, string> queryParameters = null, CancellationToken cancellationToken = default)
        {
            ...
            return await Execute(request, Method.Delete, cancellationToken).ConfigureAwait(false);
        }
        ...
        public async Task<Response<TOutput>> Get<TOutput>(string uri = null, Dictionary<string, string> urlParameters = null, Dictionary<string, string> queryParameters = null, CancellationToken cancellationToken = default)
        {
            ...
            return await Execute<TOutput>(request, Method.Get, cancellationToken).ConfigureAwait(false);
        }
    }
}
